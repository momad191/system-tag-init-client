...
>("teams/fetchTeams", async (params, thunkAPI) => {
    try {
        ...
        const response = await axios.get(`${TEAMS_URL}?page=${page}&limit=${limit}&search=${search}&sortBy=${sortBy}&sortOrder=${sortOrder}`, {
...
// Create team
export const createTeam = createAsyncThunk<Team, { name: string; members: string[]; channels: string[] }, { rejectValue: string }>(
    "teams/createTeam",
    async (body, thunkAPI) => {
        try {
            const token = Cookies.get("token");
            const response = await axios.post(TEAMS_URL, body, { headers: { Authorization: `Bearer ${token}` } });
            return response.data;
        } catch (err: any) {
            return thunkAPI.rejectWithValue(err.response?.data?.message || err.message);
        }
...
// Update team
export const updateTeam = createAsyncThunk<Team, { id: string; body: { name?: string; members?: string[]; channels?: string[] } }, { rejectValue: string }>(
    "teams/updateTeam",
    async ({ id, body }, thunkAPI) => {
        try {
            const token = Cookies.get("token");
            const response = await axios.patch(`${TEAMS_URL}/${id}`, body, { headers: { Authorization: `Bearer ${token}` } });
            return response.data;
        } catch (err: any) {
            return thunkAPI.rejectWithValue(err.response?.data?.message || err.message);
        }
...
// Delete team
export const deleteTeam = createAsyncThunk<string, string, { rejectValue: string }>("teams/deleteTeam", async (id, thunkAPI) => {
    try {
        const token = Cookies.get("token");
        await axios.delete(`${TEAMS_URL}/${id}`, { headers: { Authorization: `Bearer ${token}` } });
        return id;
    } catch (err: any) {
        return thunkAPI.rejectWithValue(err.response?.data?.message || err.message);
    }
...
const teamSlice = createSlice({
    ...
    extraReducers: (builder) => {
        ...
        builder.addCase(updateTeam.fulfilled, (state, action) => {
            ...
            if (state.selectedTeam?._id === action.payload._id) state.selectedTeam = action.payload;
        });
        ...
        builder.addCase(fetchTeamById.fulfilled, (state, action: PayloadAction<Team>) => {
            state.loading = false;
            state.currentTeam = action.payload;
        });
        builder.addCase(fetchTeamById.rejected, (state, action: PayloadAction<any>) => {
            state.loading = false;
        ...
        builder.addCase(fetchTeamsByMemberId.fulfilled, (state, action: PayloadAction<Team[]>) => {
            state.loading = false;
            state.memberTeams = action.payload;
        });
        builder.addCase(fetchTeamsByMemberId.rejected, (state, action: PayloadAction<any>) => {
            state.loading = false;
            state.error = action.payload; 
        });
